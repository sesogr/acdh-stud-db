...
$pdo = new PDO(MARIA_DSN, MARIA_USER, MARIA_PASS, [
...
$listSimilarStudents->execute([$_GET['id'], $sumPropertyWeights, $_GET['id'], $_GET['id']]);
$similarStudents = $listSimilarStudents->fetchAll();
$showDupes = count($similarStudents);
$similarIds = array_map(function ($record) {
    return $record['other_id'];
}, $similarStudents);
array_unshift($similarIds, $_GET['id']);
$listProperties->execute([sprintf(",%s,", implode(",", $similarIds))]);
$listLectures->execute([sprintf(",%s,", implode(",", $similarIds))]);
$student = [];
$hasTimes = false;
