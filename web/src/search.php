...
$pageSize = 100;
$pageNo = 0;
$pdo = new PDO(MARIA_DSN, MARIA_USER, MARIA_PASS, [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
    ...
    if ($compressed = base64_decode($_GET['token'], true)) {
        if ($serialization = gzuncompress($compressed)) {
            if ($data = unserialize($serialization)) {
                [$pageNo, $pageSize, $params, $sort, $order] = $data;
            }
        }
    }
}
if (empty($params)) {
    ...
    $sort = $sort && in_array($sort, ['birth_date', 'birth_country_historic', 'birth_country_today', 'religion', 'language', 'gender']) ? $sort : "concat_ws(' ', ascii_last_name, ascii_given_names)";
}
$query =
    /** @lang MySQL */
    <<<'EOD'
...
<?php if ($rowCount): ?>
    <p>Seite
        <?php for ($i = 0; $i < $pageCount; $i++): ?>
            ...
            <?php else: ?>
                <a href="?token=<?php
                echo urlencode(base64_encode(gzcompress(serialize([$i, $pageSize, $params, $sort, $order]))))
                ?>"><?php echo $i + 1 ?></a>
            <?php endif ?>
        <?php endfor ?>
    </p>
    <table>
        ...
        foreach ($listStudents as $student): ?>
            <tr>
                <td><?php echo htmlspecialchars(implode(', ', [$student['last_name'], $student['given_names']])); ?></td>
