...
CREATE TABLE `student_attendance` DEFAULT CHARSET utf8 AS
	SELECT DISTINCT
		...
		`class`,
		/*concat_ws(':',
			concat(ifnull(p0.merged_id, '—'), '/', ifnull(p0.student_id, '—'), '/', ifnull(p0.semester, '—')),
			concat(ifnull(p1.merged_id, '—'), '/', ifnull(p1.student_id, '—'), '/', ifnull(p1.semester, '—')),
			concat(ifnull(p2.id, '—'), '/—/', ifnull(p2.semester, '—')),
			concat(ifnull(p3.id, '—'), '/—/', ifnull(p3.semester, '—')),
			concat(ifnull(l.person_id, '—'), '/', ifnull(l.student_id, '—'), '/', ifnull(l.x_semester, '—')),
			concat('/', ifnull(l.seq_no, '—'), '/', ifnull(l.semester_rel, '—'))
		)*/
		`remarks`
	FROM (
			 (
